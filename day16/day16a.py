...
same train service (via the airport security cameras) together into a single document you can reference
(your puzzle input).
...
are much more complicated. In any case, you've extracted just the numbers in such a way that the first number
is always the same specific field, the second number is always a different specific field, and so on - you just don't
...
ticket are all valid for at least one field. This is not true of the other three nearby tickets:
the values 4, 55, and 12 are are not valid for any field. Adding together all of the invalid values
...
with open(fname, 'r') as fp:
    line = fp.readline()
    while line != '\n':
        rules.append([re.findall(': (.+) or (.+)', line.strip())[0][0],
...
# Now expand rules into full list
for rule in rules:
    n1 = int(re.findall('(.+)-', rule[0])[0])
    n2 = int(re.findall('-(.+)', rule[0])[0]) + 1
    n3 = int(re.findall('(.+)-', rule[1])[0])
    n4 = int(re.findall('-(.+)', rule[1])[0]) + 1
    print(f'found rules : n1 = {n1}, n2 = {n2}, n3 = {n3}, n4 = {n4}')
    for ticket in tickets:
        for field in ticket:
            if int(field) in range(n1, n2):
                try:
                    flat_tix.remove(int(field))
                except:
                    pass
            elif int(field) in range(n3, n4):
                try:
                    flat_tix.remove(int(field))
                except:
                    pass
...
print(f'Nearby tickets = {tickets}')
print(f'invalid tickets = {flat_tix}')
print(f'TSER = {sum(flat_tix)}')
