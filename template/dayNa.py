...
None of the currency exchanges seem to have heard of them, but somehow, you'll need to find fifty of these coins by
...
while x < len(entries):
    ...
    while y < len(entries):
        if entries[x] + entries[y] == SOLUTION:
            esc = True
            print(f'Found solution set ({entries[x]},{entries[y]}) @ ({x},{y})')
