...
validpassword = 0
inputfile = 'input.txt'
outputfile = 'day3aOut.txt'     # Generate map of path taken and write to this file
...
while row < len(entries):
    print(f'Sledding at ({row},{col}) -> \'{entries[row][col]}\'')
    strlist = list(entries[row])
    if entries[row][col] == '#':
        treecount += 1
        strlist[col] = 'X'
    else:
    ...
    col = (col + 3) % len(entries[row])
    row += 1
    wrstring = ''.join(strlist)
    wp.write(f'{wrstring}\n')
