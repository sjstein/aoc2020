...
age (if the last number is a repeat).
...
for num in nums:
    numdict[num] = turn
    turn += 1
# By definition, after the list is seeded, the last number spoken is responded to with a 0 (since it is the first time
# it is spoken), so now need to iterate on that.
lastnum = 0
while turn < 30000000:
    if lastnum in numdict:
        lastnum = findkey(numdict, lastnum, turn)
    else:
        numdict[lastnum] = turn
        lastnum = 0
    turn += 1
    if turn == 2020:
        print(f'At turn {turn} with number = {lastnum}')
print(f'At turn {turn} with last number = {lastnum}')
