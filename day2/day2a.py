...
while x < len(entries):
    ...
    minchar = entries[x].split(': ')[0].split(' ')[0].split('-')[0]
    maxchar = entries[x].split(': ')[0].split(' ')[0].split('-')[1]
    pwstring = entries[x].split(': ')[1]
    print(f'Checking {pwstring} for min {minchar} and max {maxchar} of \'{pchar}\'')
    charcount = int(pwstring.count(pchar))
    if int(minchar) <= charcount <= int(maxchar):
        print(f'Valid')
        validpassword += 1
    x += 1
