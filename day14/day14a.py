...
memory addresses are both 36-bit unsigned integers. For example, ignoring bitmasks for a moment,
...
on the left and the least significant bit (2^0, that is, the 1s bit) on the right. The current bitmask is applied to
...
inally, the program tries to write 0 to address 8:
...
with open(fname, 'r') as fp:
    for line in fp:
        if line.startswith('mask'):
            ...
            maskedval = maskvalue(maskstr, int(memval))
            print(f'found memory loc: {memloc}')
            print(f'found memory val: {memval}')
            print(f'writing val: {maskedval}')
            memdict[memloc] = [memval, maskstr, maskedval]
print(memdict)
for key in memdict:
    memspace.append([int(key), int(memdict[key][2])])
memspace.sort()
memtotal = 0
