...
for entry in entries:
    if entry.count(':') == 8:
        print(f'{entry} has 8 required fields: VALID')
        valid += 1
    elif entry.count(':') == 7 and entry.find('cid:') == -1:
        print(f'{entry} has 7 fields & missing cid: VALID')
        valid += 1
    else:
        print(f'{entry} does not have required fields: INVALID')


print(f'{valid} valid entries found')
